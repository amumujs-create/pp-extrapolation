#!/usr/bin/env python3
"""One-shot CCMR v2.0 on Multi-Stage Stage-2 TP_k win-set cohort."""
...
DATA = ROOT / "data/external/multistage_stage2_tpk/rpt_capacity.csv"
OUT = ROOT / "results/multistage_stage2_tpk_ccmr_v20"
MANIFEST = ROOT / "protocols/CCMR_V20_FROZEN_PREDICTOR_MANIFEST.json"
PROTOCOL = ROOT / "protocols/MULTISTAGE_STAGE2_TPK_CCMR_V20_PROTOCOL.md"
HISTORY = 3
HORIZON = 1
...
def load_trajectories():
    if not DATA.exists():
        raise RuntimeError(
            "missing Stage-2 extraction; run extract_multistage_stage2_tpk.py"
        )
    frame = pd.read_csv(DATA)
    trajectories = {}
    for unit, group in frame.groupby("cell"):
        if not str(unit).startswith("TP_k"):
            raise RuntimeError(f"non-TP_k cell leaked into extraction: {unit}")
...
def main():
    ...
    if result_path.exists():
        raise RuntimeError("refusing to overwrite Stage-2 TP_k one-shot")
    ...
    payload.update({
        "status": (
            "performance success" if success
            else "safe fallback or failure"
        ),
        ...
        "validation": {
            ...
            "metrics": (
                score(
                    validation, validation_candidate, validation_choose
                )
