#!/usr/bin/env python3
"""Frozen PP-X v1.0 on the Stanford BatteryLife processed cohort.

Follows protocols/STANFORD_PPX_V1_PROTOCOL.md.  Normalized health with a
25th-percentile train cutoff.  Archive SHA-256 is written before pickle
...
DATA = ROOT / "data" / "stanford_ppx" / "Stanford.zip"
OUT = ROOT / "results" / "stanford_ppx_v1"
MANIFEST = ROOT / "data" / "stanford_ppx" / "sha256_manifest.json"
...
def load_series() -> dict[str, dict]:
    ...
    with zipfile.ZipFile(DATA) as archive:
        for name in sorted(item for item in archive.namelist() if item.endswith(".pkl")):
            ...
            first5 = float(np.median(capacity[:5])) if capacity.size >= 5 else float("nan")
            out[cell] = {
...
def rows(library: dict[str, dict], names: list[str], crossings: dict[str, int],
         *, mode: str, boundary: float | None, train_strict: bool = False) -> dict:
    xs, ys, groups, health = [], [], [], []
    for name in names:
        capacity = library[name]["capacity"]
        cycle = library[name]["cycle"]
        ambient = library[name]["ambient"]
        scale = float(library[name]["first5"])
        health_series = capacity / scale
        stop = crossings[name]
        for index in range(HISTORY - 1, stop):
            window = health_series[index - HISTORY + 1:index + 1]
            value = float(health_series[index])
            rate = float(health_series[index] - health_series[index - 1])
            slope = float((window[-1] - window[0]) / max(HISTORY - 1, 1))
            if mode == "train" and boundary is not None:
                if train_strict and value <= boundary:
                    continue
                if not train_strict and value < boundary:
                    continue
...
def main() -> None:
    ...
    MANIFEST.write_text(json.dumps({
        "archive": "Stanford.zip",
        "source": "https://zenodo.org/records/14934405/files/Stanford.zip",
        "sha256": digest,
    ...
    train = rows(library, ids["train"], crossings, mode="train", boundary=cutoff, train_strict=True)
    if len(train["y"]) == 0:
        write({
            "status": "infeasible",
            "reason": "no train rows strictly above the 25th-percentile train health cutoff",
            "eligibility": audits,
            "eligible_ids": eligible,
            "split_ids": ids,
            "hull": {"train_health_q25": cutoff},
        })
        return
    ...
    if not (
        ...
        write({
            "status": "infeasible",
            "reason": "a scored outer split is empty or not strictly below the actual train health boundary",
            "model": "PP-X v1.0 core + validation-selected residual executor",
            "protocol": "STANFORD_PPX_V1",
            "archive_sha256": digest,
            "eligibility": audits,
    ...
    for width in WIDTHS:
        for lr in LEARNING_RATES:
            ...
            for name, extra in EXECUTORS.items():
                ...
                loss = float(fit.selection["best_validation_mse"])
                losses[name] = loss
                extras[name] = extra
                pp_search.append({
    ...
    write({
        ...
        "model": "PP-X v1.0 core + validation-selected residual executor",
        "dataset": "Stanford BatteryLife processed",
        "protocol": "STANFORD_PPX_V1",
        "archive_sha256": digest,
        "eligibility": audits,
        "eligible_ids": eligible,
