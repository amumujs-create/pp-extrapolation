#!/usr/bin/env python3
"""EDIST-PPX challenge on the five opened external settings."""
...
from pp_extrapolation.environment_dro_slope_transport import (
    fit_environment_dro_slope_transport,
    predict_environment_dro_slope_transport,
)

OUT = ROOT / "results" / "edist_ppx_external_development_v1"
PROTOCOL = "protocols/EDIST_PPX_EXTERNAL_DEVELOPMENT_PROTOCOL.md"
...
def select(item):
    ...
    for config in GRID:
        ...
        for train, validation in item["folds"]:
            fit = fit_environment_dro_slope_transport(
...
def evaluate(item):
    ...
    for seed in SEEDS:
        fit = fit_environment_dro_slope_transport(
            ...
            **selected["config"])
        prediction = predict_environment_dro_slope_transport(
...
def render(payload):
    lines = ["# EDIST-PPX 외부 Engression challenge 결과\n\n",
             "> EDIST 구조와 censoring-aligned validation의 retrospective 개발 결과다.\n\n",
             "| 설정 | Engression R2 | EDIST-PPX R2 | delta | 판정 |\n",
             "|---|---:|---:|---:|---|\n"]
    ...
    lines.extend(["\n## 요약\n\n", f"- 승리: {summary['wins']}/5\n",
                  ...
                  f"EDIST-PPX {summary['mean_r2']:.4f}\n",
...
def main():
    ...
    for item in load_items():
        ...
        arrays[f"{key}_truth"] = item["test"]["y"]; arrays[f"{key}_edist_ppx"] = predictions
    ...
    payload = {"model": "Censoring-aligned Innovation Slope Transport PP-X (EDIST-PPX)",
