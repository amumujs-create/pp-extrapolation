#!/usr/bin/env python3
"""Range-extract Stage-2 TP_k RPT capacities for the CCMR win-set cohort.

Refuses to run unless the frozen public manifest matches Figshare metadata.
Does not touch Stage-1 TP_z archives.
"""
...
OUT = ROOT / "data/external/multistage_stage2_tpk"
API = "https://api.figshare.com/v2/articles/25975315"
PUBLIC = ROOT / "protocols/MULTISTAGE_STAGE2_TPK_PUBLIC_MANIFEST.json"
...
def main():
    ...
    if output.exists():
        raise RuntimeError("refusing to overwrite Stage-2 TP_k extraction")
    ...
    if len(live) != len(expected):
        raise RuntimeError(
            f"expected {len(expected)} TP_k archives, found {len(live)}"
        )
