#!/usr/bin/env python3
"""MCST-PPX challenge on the five opened external settings."""
...
from pp_extrapolation.innovation_minimax_consensus_transport import (
    fit_innovation_minimax_consensus_transport,
    predict_innovation_minimax_consensus_transport,
)

OUT = ROOT / "results" / "mcst_ppx_external_development_v1"
PROTOCOL = "protocols/MCST_PPX_EXTERNAL_DEVELOPMENT_PROTOCOL.md"
...
def select(item):
    ...
    for config in GRID:
        ...
        for train, validation in item["folds"]:
            fit = fit_innovation_minimax_consensus_transport(
...
def evaluate(item):
    ...
    for seed in SEEDS:
        fit = fit_innovation_minimax_consensus_transport(
            ...
            **selected["config"])
        prediction = predict_innovation_minimax_consensus_transport(
...
def render(payload):
    lines = ["# MCST-PPX 외부 Engression challenge 결과\n\n",
             "> MCST 구조와 censoring-aligned validation의 retrospective 개발 결과다.\n\n",
             "| 설정 | Engression R2 | MCST-PPX R2 | delta | 판정 |\n",
             "|---|---:|---:|---:|---|\n"]
...
def main():
    ...
    for item in load_items():
        ...
        arrays[f"{key}_truth"] = item["test"]["y"]; arrays[f"{key}_mcst_ppx"] = predictions
    ...
    payload = {"model": "Censoring-aligned Innovation Slope Transport PP-X (MCST-PPX)",
