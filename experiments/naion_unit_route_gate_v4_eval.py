...
def main():
  ...
  for u in ids['test']:
    m=te['groups']==u;decision=unit_checks[u]['approved']
    if not np.any(m):
      per[u]={'decision':'ABSTAIN','evaluable':False,'reason':'no cycles beyond train-defined boundary'}
      continue
    pm=regression_metrics(te['y'][m],pe[m],te['groups'][m]);qm=regression_metrics(te['y'][m],qe[m],te['groups'][m]);actual=bool(qm['pooled']['r2']>0 and qm['pooled']['rmse']<=pm['pooled']['rmse']);route[m]=qe[m] if decision else pe[m];correct.append(decision==actual);approved.extend(np.flatnonzero(m).tolist() if decision else []);per[u]={'decision':'PP' if decision else 'MLP','evaluable':True,'actual_pp_success':actual,'correct':decision==actual,'plain':pm,'pp':qm}
  ...
  result={'status':'prospective one-shot external confirmation; empty-tail handling repair after frozen gate decision','development_source':'group-3 mixed unit response; no group-4 outcome used','gate':gate,'n':{'train':len(tr['y']),'validation':len(va['y']),'test':len(te['y'])},'projected_all':{'plain':regression_metrics(te['y'],pe,te['groups']),'pp':regression_metrics(te['y'],qe,te['groups'])},'per_unit_route':per,'unit_route_accuracy':float(np.mean(correct)) if correct else None,'selective_pp_metrics':sel,'deployment_route_metrics':regression_metrics(te['y'],route,te['groups']),'gate_success':bool(all(correct) and gate['coverage']>0),'correct_rejection_without_coverage':bool(all(correct) and gate['coverage']==0)}
