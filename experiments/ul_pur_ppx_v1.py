#!/usr/bin/env python3
"""Frozen PP-X v1.0 on the UL-PUR BatteryLife processed cohort.

Follows protocols/UL_PUR_PPX_V1_PROTOCOL.md.  Archive SHA-256 was recorded
before pickle parsing.  Test labels are used only after executor and
hyperparameter selection on validation RMSE.
"""
...
DATA = ROOT / "data" / "ul_pur_ppx" / "UL_PUR.zip"
OUT = ROOT / "results" / "ul_pur_ppx_v1"
...
def main() -> None:
    ...
    digest = sha256(DATA)
    expected = "da2d5815436998286b6628c8a50b186e70f5a9a0955a29b1ae6a1190084f02f2"
    if digest != expected:
        raise RuntimeError(f"UL_PUR.zip hash {digest} != recorded {expected}")
    ...
    write({
        ...
        "model": "PP-X v1.0 core + validation-selected residual executor",
        "dataset": "UL-PUR BatteryLife processed",
        "protocol": "UL_PUR_PPX_V1",
        "archive_sha256": digest,
        "eligibility": audits,
        "eligible_ids": eligible,
