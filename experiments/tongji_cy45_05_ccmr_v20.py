#!/usr/bin/env python3
"""One-shot CCMR v2.0 on Tongji CY45-05 win-set cohort."""
...
DATA = ROOT / "data/external/tongji_cy45_05/capacity.csv"
OUT = ROOT / "results/tongji_cy45_05_ccmr_v20"
...
SPLIT_SALT = "tongji-cy45-05-ccmr-v20:"
MIN_CYCLES = 50
...
def load_trajectories():
    if not DATA.exists():
        raise RuntimeError(
            "missing Tongji extraction; run extract_tongji_cy45_05.py"
        )
    frame = pd.read_csv(DATA)
    trajectories = {}
    for unit, group in frame.groupby("cell"):
        if "CY45-05" not in str(unit):
            raise RuntimeError(f"non-CY45-05 cell leaked: {unit}")
        ...
        if len(capacity) < MIN_CYCLES or capacity[0] <= 0:
            continue
...
def main():
    ...
    if result_path.exists():
        raise RuntimeError("refusing to overwrite Tongji CY45-05 one-shot")
