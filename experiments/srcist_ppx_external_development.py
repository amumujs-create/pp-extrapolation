#!/usr/bin/env python3
"""SRCIST-PPX challenge on the five opened external settings."""
...
from pp_extrapolation.innovation_source_robust_transport import (
    fit_innovation_source_robust_transport,
    predict_innovation_source_robust_transport,
)

OUT = ROOT / "results" / "srcist_ppx_external_development_v1"
PROTOCOL = "protocols/SRCIST_PPX_EXTERNAL_DEVELOPMENT_PROTOCOL.md"
...
def select(item):
    ...
    for config in GRID:
        ...
        for train, validation in item["folds"]:
            fit = fit_innovation_source_robust_transport(
...
def evaluate(item):
    ...
    for seed in SEEDS:
        fit = fit_innovation_source_robust_transport(
            ...
            **selected["config"])
        prediction = predict_innovation_source_robust_transport(
...
def render(payload):
    lines = ["# SRCIST-PPX 외부 Engression challenge 결과\n\n",
             "> SRCIST 구조와 censoring-aligned validation의 retrospective 개발 결과다.\n\n",
             "| 설정 | Engression R2 | SRCIST-PPX R2 | delta | 판정 |\n",
             "|---|---:|---:|---:|---|\n"]
    ...
    lines.extend(["\n## 요약\n\n", f"- 승리: {summary['wins']}/5\n",
                  ...
                  f"SRCIST-PPX {summary['mean_r2']:.4f}\n",
...
def main():
    ...
    for item in load_items():
        ...
        arrays[f"{key}_truth"] = item["test"]["y"]; arrays[f"{key}_srcist_ppx"] = predictions
    ...
    payload = {"model": "Censoring-aligned Innovation Slope Transport PP-X (SRCIST-PPX)",
