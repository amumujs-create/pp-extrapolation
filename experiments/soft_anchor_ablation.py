...
def evaluate(train, validation, test, seeds, anchors):
    ...
    for seed in seeds:
        ...
        for anchor in anchors:
            fit = fit_pp(train, validation, seed=seed, affine_selection=affine,
                         affine_anchor_weight=anchor)
            candidates.append((fit.selection["best_validation_mse"], anchor, fit))
        _, selected_anchor, selected_fit = min(candidates, key=lambda z: (z[0], -z[1]))
        ...
        rows.append({
            "seed": seed, "selected_anchor": selected_anchor,
            "candidate_validation_mse": {str(a): float(v) for v, a, _ in candidates},
            "soft_anchor": soft_metrics, "frozen_pp": frozen_metrics,
        })
        print(f"seed={seed} anchor={selected_anchor:g} soft={soft_metrics['pooled']['r2']:.4f} frozen={frozen_metrics['pooled']['r2']:.4f}", flush=True)
    def summary(key):
...
def evaluate_folds(folds, seeds, anchors):
    rows = []
    for seed in seeds:
        ...
        for fold in folds:
            ...
            for anchor in anchors:
                ...
                candidates.append((fit.selection["best_validation_mse"], anchor, fit))
            _, anchor, fit = min(candidates, key=lambda z: (z[0], -z[1]))
            ...
            chosen.append({"test_cell": fold["test_cell"], "anchor": anchor})
...
def main():
    ...
    if args.dataset == "nasa":
        ...
        result = evaluate_folds(folds, seeds, anchors)
    else:
        prepared = prepare_hust() if args.dataset == "hust" else prepare_virkler()
        result = evaluate(*prepared[:3], seeds, anchors)
