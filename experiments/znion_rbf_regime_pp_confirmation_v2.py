...
def main():
 ...
 prior_hash=hashes_under([ROOT/'data/znion_bq_confirmatory/test',ROOT/'data/znion_regime_scale_confirmation_v1']);duplicates={p.stem:prior_hash[hashlib.sha256(p.read_bytes()).hexdigest()] for p in paths if hashlib.sha256(p.read_bytes()).hexdigest() in prior_hash};raw={p.stem:data.read_cell(p) for p in paths};right=[u for u,c in raw.items() if c is None];cells={u:c for u,c in raw.items() if c is not None and u not in duplicates}
