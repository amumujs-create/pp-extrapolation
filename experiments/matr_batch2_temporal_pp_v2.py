#!/usr/bin/env python3
"""Final-PP backbone plus a bounded causal GRU residual on MATR batch 2."""
...
TEMPORAL = tuple({"width": width, "bound": bound, "decay": decay, "lr": lr, "wd": wd}
                 for width, lr, wd in ((16, 1e-3, 0.0), (32, 5e-4, 0.1), (64, 5e-4, 0.1))
                 for bound, decay in ((0.10, 0.0), (0.25, 0.0), (0.50, 0.0), (1.0, 0.0), (0.50, 0.05)))
...
def fit_temporal(config, seed, sequence, base, target, group, validation):
    ...
    for epoch in range(1, 251):
        ...
        for start in range(0, len(x), 512):
            ...
            loss = torch.mean(w[index] * (model(x[index], b[index]) - y[index]) ** 2)
...
def main():
    ...
    output = ROOT / "results" / "matr_batch2_temporal_pp_v2"; output.mkdir(parents=True, exist_ok=True)
    ...
    result = {"status": "post-hoc development", "model": "final PP + bounded causal GRU residual",
