...
def main():
 ...
 pp=np.asarray(pp);plain=np.asarray(plain);ppm=regression_metrics(test['y'],pp.mean(0),test['groups']);plm=regression_metrics(test['y'],plain.mean(0),test['groups']);result={'status':'one-shot untouched confirmation','frozen_from_commit':'7edc6ae','locked_files':EXPECTED,'right_censored':right_censored,'no_evaluable_tail':no_tail,'n_evaluation_units':len(np.unique(test['groups'])),'n_rows':len(test['y']),'runs':runs,'pp_mean':ppm,'pp_median':regression_metrics(test['y'],np.median(pp,axis=0),test['groups']),'plain_mean':plm,'success':bool(ppm['pooled']['r2']>0 and ppm['pooled']['r2']>plm['pooled']['r2']),'criteria':'PP mean pooled R2 > 0 and > plain MLP mean pooled R2'}
