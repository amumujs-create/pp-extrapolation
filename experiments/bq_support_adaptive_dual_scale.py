#!/usr/bin/env python3
"""Support-adaptive dual-scale saturation in one BQ-PP residual head."""
...
OUT=ROOT/'results/bq_support_adaptive_dual_scale_v1';SEEDS=(42,43,44,45,46)
CONFIGS=[{'broad_residual_bound':b,'local_saturation_weight':w,'support_gate_threshold':t,'support_gate_temperature':temp} for b in (6.,10.) for w in (.2,.4) for t in (.5,1.) for temp in (.25,.5)]
BASE=dict(width=64,alpha=1000.,learning_rate=1e-3,weight_decay=.01,residual_bound=2.,support_gate_feature=2,support_adaptive_saturation=True)
...
def main():
 ...
 for x in finalists:
  c={k:x[k] for k in ('broad_residual_bound','local_saturation_weight','support_gate_threshold','support_gate_temperature')};runs=[];preds=[]
  for seed in SEEDS:
 ...
 chosen=max(final,key=lambda x:(x['worst_seed_dataset_r2'],x['ensemble_min_dataset_r2'],x['ensemble_mean_dataset_r2']));result={'status':'retrospective source-informed unified PP development; requires frozen external confirmation','objective':'maximize worst single-seed dataset pooled R2, then ensemble minimum and mean','model':'support-adaptive dual-scale saturation BQ-PP','base':BASE,'screen':screen,'finalists':final,'selected':chosen,'global_envelope':'abs(yhat-y_affine) <= margin * broad_residual_bound','data_audits':audits,'runtime_seconds':time.perf_counter()-start};(OUT/'results.json').write_text(json.dumps(result,indent=2)+'\n')
